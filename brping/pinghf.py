...
class Pinghf(PingDevice):

    def legacyRequest(self, m_id, timeout=1.2):
    ...
    def initialize(self):
        if not PingDevice.initialize(self):
            return False
        if self.legacyRequest(definitions.PINGHF_GENERAL_INFO) is None:
            return False
        return True
    ...
    def get_device_id(self):
        if self.legacyRequest(definitions.PINGHF_DEVICE_ID) is None:
            return None
        data = ({
    ...
    def get_distance(self):
        if self.legacyRequest(definitions.PINGHF_DISTANCE) is None:
            return None
        data = ({
    ...
    def get_distance2(self):
        if self.legacyRequest(definitions.PINGHF_DISTANCE2) is None:
            return None
        data = ({
            "distance_mm": self._distance_mm,  # 
            "avg_distance_mm": self._avg_distance_mm,  # 
    ...
    def get_distance_simple(self):
        if self.legacyRequest(definitions.PINGHF_DISTANCE_SIMPLE) is None:
            return None
        data = ({
    ...
    def get_firmware_version(self):
        if self.legacyRequest(definitions.PINGHF_FIRMWARE_VERSION) is None:
            return None
        data = ({
    ...
    def get_gain_setting(self):
        if self.legacyRequest(definitions.PINGHF_GAIN_SETTING) is None:
            return None
        data = ({
            "gain_setting": self._gain_setting,  # The current gain setting. 0: 0.6, 1: 1.8, 2: 5.5, 3: 12.9, 4: 30.2, 5: 66.1, 6: 144
        })
    ...
    def get_mode_auto(self):
        if self.legacyRequest(definitions.PINGHF_MODE_AUTO) is None:
            return None
        data = ({
    ...
    def get_pcb_temperature(self):
        if self.legacyRequest(definitions.PINGHF_PCB_TEMPERATURE) is None:
            return None
        data = ({
    ...
    def get_ping_enable(self):
        if self.legacyRequest(definitions.PINGHF_PING_ENABLE) is None:
            return None
        data = ({
    ...
    def get_ping_interval(self):
        if self.legacyRequest(definitions.PINGHF_PING_INTERVAL) is None:
            return None
        data = ({
    ...
    def get_processor_temperature(self):
        if self.legacyRequest(definitions.PINGHF_PROCESSOR_TEMPERATURE) is None:
            return None
        data = ({
    ...
    def get_profile(self):
        if self.legacyRequest(definitions.PINGHF_PROFILE) is None:
            return None
        data = ({
            "distance": self._distance,  # Units: mm; The current return distance determined for the most recent acoustic measurement.
            "confidence": self._confidence,  # Units: %; Confidence in the most recent range measurement.
    ...
    def get_profile6_t(self):
        if self.legacyRequest(definitions.PINGHF_PROFILE6_T) is None:
            return None
        data = ({
            "ping_number": self._ping_number,  # 
            "start_mm": self._start_mm,  # 
            "length_mm": self._length_mm,  # 
            "start_ping_hz": self._start_ping_hz,  # 
            "end_ping_hz": self._end_ping_hz,  # 
            "adc_sample_hz": self._adc_sample_hz,  # 
            "timestep_msec": self._timestep_msec,  # 
            "spare2": self._spare2,  # 
            "ping_duration_sec": self._ping_duration_sec,  # 
            "analog_gain": self._analog_gain,  # 
            "max_pwr": self._max_pwr,  # 
            "min_pwr": self._min_pwr,  # 
            "step_db": self._step_db,  # 
            "fspare2": self._fspare2,  # 
            "is_db": self._is_db,  # 
            "gain_index": self._gain_index,  # 
            "decimation": self._decimation,  # 
            "reserved": self._reserved,  # 
            "num_results": self._num_results,  # 
            "pwr_results": self._pwr_results,  # 
        })
        return data
    ...
    def get_range(self):
        if self.legacyRequest(definitions.PINGHF_RANGE) is None:
            return None
        data = ({
    ...
    def get_speed_of_sound(self):
        if self.legacyRequest(definitions.PINGHF_SPEED_OF_SOUND) is None:
            return None
        data = ({
    ...
    def get_transmit_duration(self):
        if self.legacyRequest(definitions.PINGHF_TRANSMIT_DURATION) is None:
            return None
        data = ({
    ...
    def get_voltage_5(self):
        if self.legacyRequest(definitions.PINGHF_VOLTAGE_5) is None:
            return None
        data = ({
    ...
    def set_device_id(self, device_id, verify=True):
        m = pingmessage.PingMessage(definitions.PINGHF_SET_DEVICE_ID)
    ...
    def set_gain_setting(self, gain_setting, verify=True):
        m = pingmessage.PingMessage(definitions.PINGHF_SET_GAIN_SETTING)
    ...
    def set_mode_auto(self, mode_auto, verify=True):
        m = pingmessage.PingMessage(definitions.PINGHF_SET_MODE_AUTO)
    ...
    def set_ping_enable(self, ping_enabled, verify=True):
        m = pingmessage.PingMessage(definitions.PINGHF_SET_PING_ENABLE)
    ...
    def set_ping_interval(self, ping_interval, verify=True):
        m = pingmessage.PingMessage(definitions.PINGHF_SET_PING_INTERVAL)
    ...
    def set_range(self, scan_start, scan_length, verify=True):
        m = pingmessage.PingMessage(definitions.PINGHF_SET_RANGE)
    ...
    def set_speed_of_sound(self, speed_of_sound, verify=True):
        m = pingmessage.PingMessage(definitions.PINGHF_SET_SPEED_OF_SOUND)
